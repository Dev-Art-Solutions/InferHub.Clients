...
class InferHubClient(_CorpusMethodsMixin):
    ...
    def upsert(self, collection: str, upsert: VectorUpsert) -> VectorRecord:
    ...
    def query(self, collection: str, query: VectorQuery) -> list:
    ...
    def retrieve(self, collection: str, query: VectorQuery) -> list:
    ...
    def get_record(self, collection: str, id: str) -> Optional[VectorRecord]:
        """``GET /api/vector/{collection}/{id}`` — ``None`` on 404, never raised."""
        response = self._http.get(f"api/vector/{collection}/{id}")
        if response.status_code == 404:
            return None
        raise_for_status(response)
        return VectorRecord.from_json(response.json())

    def delete_record(self, collection: str, id: str) -> bool:
        """``DELETE /api/vector/{collection}/{id}`` — ``True`` iff a record was actually deleted."""
        response = self._http.delete(f"api/vector/{collection}/{id}")
        if response.status_code == 404:
            return False
        raise_for_status(response)
        return True
